---
title: 'So Many Species, So Little Time: Making Sense of Biodiversity'
author: "Margaret Slein, Jory Griffiths, Egor Katkov (Living Data Project)"
date: "`r format(Sys.time(), '%d %B, %Y')`"
description: "How to measure, visualize, and compare biodiversity"
output: 
  html_document:
    toc: true
    toc_float: true
...
They can decompose terrestrial matter such as leaves, or consume biofilm growing on rocks within their streams.
...
Streamside logging affects many stream variables such as temperature, light exposure, leaf litter deposition, and biofilm growth. Because benthic invertebrates are affected by these variables, we can hypothesize that the biodiversity of benthic invertebrate communities might be affected by logging.
...
Interestingly, the Simpson index appears to increase over time. This is likely because Simpson's index gives a higher weight to the more dominant species, so the value can actually increase even when biodiversity is decreasing.

## Evenness Index (E)

Additionally, we can think about the evenness of species across a given area. Evenness is a metric for assessing species dominance, if evenness is high, it means most species are of equal dominance. If evenness if low, it means some species are more dominant than others
...
evenness<-  sim %>%
    ...
    mutate(richness= case_when(time %in% c("2001", "2009") ~ 9, 
           ...
           prop = n/sum(n),
    shannon= sum(prop/log(prop)), 
    evenness = shannon/log(richness)) %>% # Calculate Shannon's evenness
...
# Load packages
First we need to load the packages we will be using. We will use the `codyn` package for this tutorial. We will also use `ggplot2` and `dplyr` for plotting and cleaning.
